...
<div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <button
            ...
            @click="accounts.length && openCreateModal()"
            :class="accounts.length === 0
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed border-gray-300'
                : 'bg-main text-white border-main hover:bg-main-hover'"
            class="inline-flex items-center gap-2 rounded-lg border px-4 py-3 text-theme-sm font-medium transition">
            {{ __('sentence.add_income') }}
        </button>
    </div>
</div>
