...
<div x-data="{openDropDown: false}" class="relative h-fit">
    ...
    <div x-show="openDropDown" @click.outside="openDropDown = false" 
        ...
        @forelse($items as $item)
            <button
             @class([
                'flex w-full px-3 py-2 font-medium text-left rounded-lg text-theme-xs hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-white/5 dark:hover:text-gray-300',
                'text-red-500 hover:text-red-600 dark:text-red-500 dark:hover:text-red-500' => $item === 'Delete',
                'text-gray-500' => $item !== 'Delete'
            ])>
                {{ $item }}
            </button>
        @empty
